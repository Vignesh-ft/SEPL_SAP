...
`;
  ...
  function hourFilterWithMissingHours(data) {
    ...
    data.forEach((datum) => {
      let hour = datum.time_cycle_end.split(":")[0]; // Extract hour from time
 ...
 const getMonthlyDataForRange = async (stationName, fromMonth, fromYear, toMonth, toYear) => {
    const query = `
      SELECT EXTRACT(MONTH FROM CAST(date AS DATE)) AS month, 
       ...
       SUM(CAST(ok_count AS INTEGER)) AS ok_count
FROM ${stationName}
WHERE (EXTRACT(YEAR FROM CAST(date AS DATE)) > $1 OR 
       (EXTRACT(YEAR FROM CAST(date AS DATE)) = $1 AND EXTRACT(MONTH FROM CAST(date AS DATE)) >= $2))
  AND (EXTRACT(YEAR FROM CAST(date AS DATE)) < $3 OR 
       (EXTRACT(YEAR FROM CAST(date AS DATE)) = $3 AND EXTRACT(MONTH FROM CAST(date AS DATE)) <= $4))
GROUP BY year, month
ORDER BY year, month;
