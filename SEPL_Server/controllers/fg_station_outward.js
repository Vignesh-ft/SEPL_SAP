...
router.post('/:stationName/hourly', async (req, res) => {
  ...
  const fetchTableForDay = async (stationName, fromDate, toDate) => {
    ...
    const query = `
      SELECT outward_date, COUNT(id) AS count
      FROM ${stationName}
...
function dateFilterWithMissingDates(data, fromDate, toDate) {
    ...
    // Aggregate the data
    data.forEach((datum) => {
        const date = formatDate(new Date(datum.outward_date)); // Convert to YYYY-MM-DD format
...
const getData = async (stationName, date) => {
  const query = `
    ...
    FROM ${stationName}
    WHERE outward_date::date = $1  -- Replace date with outward_date for filtering
